...
export const ApiDocsPage = () => {
  ...
  const testApiEndpoint = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`https://udituntwllttyejobufm.supabase.co/functions/v1/pricing-api?from=Nigeria&to=Ghana&weight=2&packageType=Documents`, {
        headers: {
  ...
  const codeExamples = {
    curl: `curl -X GET "https://udituntwllttyejobufm.supabase.co/functions/v1/pricing-api?from=Nigeria&to=Ghana&weight=2&packageType=Documents" \\
  -H "Authorization: Bearer YOUR_API_KEY" \\
  -H "Content-Type: application/json"`,
    
    javascript: `const response = await fetch(
  'https://udituntwllttyejobufm.supabase.co/functions/v1/pricing-api?from=Nigeria&to=Ghana&weight=2&packageType=Documents',
  {
    headers: {
            ...
            <CardContent className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {[
                    { name: 'from', type: 'string', required: true, description: 'Origin country (e.g., "Nigeria")' },
                    { name: 'to', type: 'string', required: true, description: 'Destination country (e.g., "Ghana")' },
                    { name: 'weight', type: 'number', required: true, description: 'Package weight in kilograms' },
                    { name: 'packageType', type: 'string', required: true, description: 'Type of package (e.g., "Documents", "Packages")' },
                  ].map((param) => (
